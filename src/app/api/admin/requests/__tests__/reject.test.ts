...
import { POST } from '@/app/api/admin/requests/[id]/reject/route'

function makeRequest(body: Record<string, unknown>) {
  return new Request('http://localhost:3000/api/admin/requests/req-1/reject', {
    method: 'POST',
...
describe('POST /api/admin/requests/[id]/reject', () => {
  ...
  it('returns 401 when user is not admin', async () => {
    ...
    const response = await POST(
      makeRequest({ rejection_reason: 'Invalid' }),
      { params: mockParams }
    )
    expect(response.status).toBe(401)
  })
  ...
  it('rejects request successfully', async () => {
    ...
    const response = await POST(
      makeRequest({
        ...
        admin_notes: 'Please resubmit with receipts',
      }),
      { params: mockParams }
    )
    const body = await response.json()
    ...
    expect(body.message).toBe('Request rejected successfully')

    // Should create notification with rejection reason
    ...
    // Should send email notification with reason
    expect(mockNotify).toHaveBeenCalledWith(
      'req-1',
      'rejected',
      'Please resubmit with receipts',
      'Missing receipts'
    )
  })
  ...
  it('returns 500 when database update fails', async () => {
    ...
    const response = await POST(
      makeRequest({ rejection_reason: 'Bad request' }),
      { params: mockParams }
    )
